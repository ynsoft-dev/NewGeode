...
@if (session()->has('Add'))
    <div class="alert alert-success alert-dismissible fade show" role="alert">
        ...
        <button type="button" class="close" data-dismiss="alert" aria-label="{{ __('boxflow.close') }}">
        ...
        <tr>
            ...
            <td>
                <a class="btn btn-xs btn-default text-primary mx-1 shadow" title="{{ __('boxflow.update') }}"
                    data-effect="effect-scale" data-id="{{ $shelf->id }}" data-name="{{ $shelf->name }}"
                    data-site_name="{{ $shelf->site->name }}" data-toggle="modal" href="#exampleModal2"><i
                        class="fa fa-lg fa-fw fa-pen"></i></a>
                <a class="btn btn-xs btn-default text-danger mx-1 shadow" title="{{ __('boxflow.update') }}"
                    data-effect="effect-scale" data-id="{{ $shelf->id }}" data-name="{{ $shelf->name }}"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">{{ __('boxflow.addShelf') }}</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="{{ __('boxflow.close') }}">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="{{ route('shelves.store') }}" method="post" autocomplete="off">
                    ...
                    <div class="form-group">
                        <label for="exampleInputEmail1">{{ __('boxflow.nameShelf') }}</label>
                    ...
                    <label class="my-1 mr-2" for="inlineFormCustomSelectPref">{{ __('boxflow.ray') }}</label>
                    <select name="ray" id="ray" class="form-control" required>
                    </select>

                    <label class="my-1 mr-2" for="inlineFormCustomSelectPref">{{ __('boxflow.column') }}</label>
                    <select name="column" id="column" class="form-control" required>
                    </select>

                    <div class="form-group">
                        <label for="capacity">{{ __('boxflow.capacity') }}</label>
...
<div class="modal" id="modaldemo8">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content modal-content-demo">
            <div class="modal-header">
                <h6 class="modal-title">{{ __('boxflow.deleteShelf') }}</h6><button
                    aria-label="{{ __('boxflow.close') }}" class="close" data-dismiss="modal" type="button"><span
            ...
            <form action="shelves/destroy" method="post">
                ...
                {{ csrf_field() }}
                <div class="modal-body">
                    <p>{{ __('boxflow.confirmDeleteShelf') }}</p><br>
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (selectedRayId) {
                                ...
                                console.log(
                                    "Déclenchement de la requête AJAX pour récupérer les colonnes"
                                );
                                $.ajax({
        ...
        if (selectedRayId) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    $('select[name="column"]').empty();
                    $.each(data, function(id, name) {
                        $('select[name="column"]').append('<option value="' + id + '">' +
                            name + '</option>');
                    });
